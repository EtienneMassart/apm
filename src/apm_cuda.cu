...
__global__ void kernel_calcul(int n_bytes, int size_pattern, int approx_factor,
                              char *buf, char *pattern, int *distances) {
...
void cas1_Cuda(int nb_patterns, char **pattern, int n_bytes, int approx_factor,
               char *buf, int *n_matches) {
    for (int i = 0; i < nb_patterns; i++) {
        ...
        if (size_pattern > 20) {
            ...
            cudaMemcpy(d_buf, buf, n_bytes * sizeof(char),
                       cudaMemcpyHostToDevice);
            cudaMemcpy(d_pattern, pattern[i], size_pattern * sizeof(char),
                       cudaMemcpyHostToDevice);

            dim3 Db = dim3(256, 1, 1);
            dim3 Dg = dim3((n_bytes + Db.x - 1) / Db.x, 1, 1);

            kernel_calcul<<<Dg, Db>>>(n_bytes, size_pattern, approx_factor,
                                      d_buf, d_pattern, d_distances);

            cudaMemcpy(distances, d_distances, n_bytes * sizeof(int),
